            ...
            for col in range(3, 8): 
                ...
                for r_idx in range(d_row + 10, len(df)):
                    ...
                    if "熱量" in label and (val < current_std["熱量"][0] or val > current_std["熱量"][1]):
                        ...
                        logs.append({"分頁": sn, "日期": date_str, "項目": "熱量", "原因": f"粉底：{val} Kcal (對標{sn})"})
                ...
                if any(day in day_name for day in ["週一", "週二", "週四"]):
                    for r_idx in range(d_row + 2, d_row + 12):
                        ...
                        if "●" in txt or "🌶️" in txt:
                            ...
                            logs.append({"分頁": sn, "日期": date_str, "項目": "禁辣日", "原因": "淺綠底違規"})
...
# --- 介面 ---
st.title("🛡️ 輕食區(新北食品) 全學部菜單稽核系統")
st.caption("製作者：Alison")
st.info("💡 系統會自動根據分頁名稱（幼兒園、小學、素食、美食街）切換合約紅線標準。")

up = st.file_uploader("👉 上傳整合式菜單 Excel", type=["xlsx"])
if up:
